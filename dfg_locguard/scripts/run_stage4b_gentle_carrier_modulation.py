...
GROUP_KEYS = ["asem_mode", "modulation_variant", "asem_alpha", "tau", "smooth_sigma", "case_key"]
...
class ModulationCase:
    ...
    @property
    def key(self) -> str:
        parts = [self.variant, f"alpha_{format_number(self.alpha)}"]
        if self.tau > 0.0:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Stage 4B gentle fragile carrier modulation ablation without training."
    )
    ...
    parser.add_argument("--output_dir", default="dfg_locguard/outputs/stage4b_gentle_carrier_modulation")
    ...
    parser.add_argument("--asem_alpha", nargs="*", type=float, default=[0.1, 0.25, 0.5])
    parser.add_argument("--tau", nargs="*", type=float, default=[0.03, 0.05, 0.08])
...
def build_cases(args: argparse.Namespace) -> list[ModulationCase]:
    cases = [ModulationCase("baseline", 0.0, 0.0, 0.0, is_baseline=True)]
    for alpha in args.asem_alpha:
        cases.append(ModulationCase("naive_boost", alpha, 0.0, 0.0))
        cases.append(ModulationCase("energy_normalized_boost", alpha, 0.0, 0.0))
        for tau in args.tau:
            cases.append(ModulationCase("clipped_boost", alpha, tau, 0.0))
            cases.append(ModulationCase("clipped_energy_normalized_boost", alpha, tau, 0.0))
        for sigma in args.smooth_sigma:
            cases.append(ModulationCase("smooth_boost", alpha, 0.0, sigma))
    return cases
...
def build_asem_map(mode: str, gt_mask: np.ndarray, size: tuple[int, int]) -> np.ndarray:
    ...
    if mode != "gt_mask_boost":
        raise ValueError(f"Stage 4B only supports gt_mask_boost oracle A_sem, got: {mode}")
...
def effective_visual_map(asem_map: np.ndarray, case: ModulationCase) -> np.ndarray:
    if case.is_baseline:
        return np.zeros_like(asem_map, dtype=np.float32)
    if case.variant == "smooth_boost" and case.smooth_sigma > 0.0:
...
def process_case(
    ...
    try:
        ...
        formula = {
            ...
            "naive_boost": "out_y_mod = host + (1 + alpha * A_sem) * residual",
            "clipped_boost": "residual_mod = clip((1 + alpha * A_sem) * residual, -tau, tau); out_y_mod = host + residual_mod",
            "energy_normalized_boost": "residual_mod = boosted_residual * mean_abs(residual) / mean_abs(boosted_residual)",
            "smooth_boost": "A_sem_smooth = gaussian_blur(A_sem, sigma); out_y_mod = host + (1 + alpha * A_sem_smooth) * residual",
            "clipped_energy_normalized_boost": "boost, clip, then energy normalize residual before adding to host",
        }[case.variant]
        meta = {
            "image_id": image_id,
            "stage": "stage4b_gentle_carrier_modulation",
            "asem_mode": args.asem_mode,
            "modulation_variant": case.variant,
            "asem_alpha": case.alpha,
            "tau": case.tau,
            ...
            "notes": [
                ...
                "GT mask is used only as the oracle A_sem modulation map and deterministic tamper region.",
...
def group_summary(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for key, group_rows in sorted(groups.items(), key=lambda item: (item[0][1], item[0][2], item[0][3], item[0][4])):
        mode, variant, alpha, tau, sigma, case_key = key
        item: dict[str, Any] = {
            "asem_mode": mode,
...
def pareto_candidates(groups: list[dict[str, Any]], baseline: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in groups:
        ...
        bit_ok = float(row["mean_bit_accuracy"]) >= 0.999
        ...
        if iou_ok and dice_ok and psnr_ok and bit_ok and (frag_ok or small_ok):
            item = dict(row)
            ...
            reasons = []
            if frag_ok:
                reasons.append("lower_fragmentation_ratio")
            if small_ok:
...
def main() -> None:
                ...
                failed_cases.append(
                    {
                        ...
                        "failure_stage": "stage4b_process_case",
    ...
    case_csv = output_dir / "stage4b_case_metrics.csv"
    write_csv(case_csv, rows, CASE_FIELDS)
    group_rows = group_summary(rows)
    group_csv = output_dir / "stage4b_group_summary.csv"
    ...
    pareto_csv = output_dir / "stage4b_pareto_candidates.csv"
    ...
    contact_info = build_contact_sheet(vis_paths, output_dir / "stage4b_contact_sheet.png", args.contact_sheet_samples)
    ...
    summary_payload = {
        ...
        "negative_result_note": (
            "No gentle variant reduced fragmentation under the Pareto constraints."
            if negative_result
            else "At least one gentle variant satisfied the Pareto constraints."
        ),
        "notes": [
            "This ablation does not train EditGuard.",
            "gt_mask_boost is an oracle modulation map used only to test carrier modulation strategy limits.",
            "GT mask is not used as the predicted mask.",
            "No Stable Diffusion, SAM, CLIP, or DINO module is used.",
            "The robust bitencoder/bitdecoder architecture is unchanged.",
        ],
    ...
    (output_dir / "stage4b_summary.json").write_text(
    ...
    print(f"Saved summary JSON: {output_dir / 'stage4b_summary.json'}")
    print(f"Saved failed cases: {failed_path}")
    print(f"Saved contact sheet: {output_dir / 'stage4b_contact_sheet.png'}")
