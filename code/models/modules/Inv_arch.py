...
class VSN(nn.Module):
    ...
    def forward(self, x, x_h=None, message=None, rev=False, hs=[], direction='f',
                asem_map=None, asem_alpha=0.0, asem_mode="none",
                asem_variant="naive_boost", asem_tau=0.0, asem_smooth_sigma=0.0,
                return_fragile_carrier=False):
        if not rev:
            if self.mode == "image":
                out_y, out_y_h = self.irn(x, x_h, rev)
                out_y = iwt(out_y)
                fragile_carrier = out_y
                host_image = iwt(x) if (asem_map is not None and asem_mode != "none") or return_fragile_carrier else None
                residual = fragile_carrier - host_image if host_image is not None else None
                if asem_map is not None and asem_mode != "none":
                    strength_map = asem_map.to(device=out_y.device, dtype=out_y.dtype)
                    if strength_map.dim() == 3:
                        strength_map = strength_map.unsqueeze(1)
                    if strength_map.shape[-2:] != out_y.shape[-2:]:
                        strength_map = F.interpolate(strength_map, size=out_y.shape[-2:], mode="bilinear", align_corners=False)
                    strength_map = torch.clamp(strength_map, 0.0, 1.0)
                    if asem_mode == "uniform":
                        strength_map = torch.zeros_like(strength_map)
                    if asem_variant in ("smooth_boost", "soft_background_suppression") and float(asem_smooth_sigma) > 0.0:
                        sigma = float(asem_smooth_sigma)
                        radius = max(1, int(round(3.0 * sigma)))
                        coords = torch.arange(-radius, radius + 1, device=strength_map.device, dtype=strength_map.dtype)
                        ...
                        strength_map = F.pad(strength_map, (radius, radius, radius, radius), mode="reflect")
                        strength_map = F.conv2d(strength_map, kernel_2d)
                        strength_map = torch.clamp(strength_map, 0.0, 1.0)

                    if asem_variant in (
                        "background_suppression",
                        "soft_background_suppression",
                        "area_reallocation",
                    ):
                        residual_mod = residual * (1.0 - float(asem_alpha) * (1.0 - strength_map))
                        if asem_variant == "area_reallocation":
                            base_energy = residual.abs().mean(dim=(1, 2, 3), keepdim=True)
                            mod_energy = residual_mod.abs().mean(dim=(1, 2, 3), keepdim=True)
                            residual_mod = residual_mod * (base_energy / torch.clamp(mod_energy, min=1e-8))
                    elif asem_variant == "residual_floor_suppression":
                        tau = float(asem_tau)
                        small_residual = (residual.abs() < tau).to(residual.dtype) if tau > 0.0 else torch.ones_like(residual)
                        residual_mod = residual * (1.0 - float(asem_alpha) * (1.0 - strength_map) * small_residual)
                    else:
                        residual_mod = (1.0 + float(asem_alpha) * strength_map) * residual
                        if asem_variant in ("clipped_boost", "clipped_energy_normalized_boost") and float(asem_tau) > 0.0:
                            residual_mod = torch.clamp(residual_mod, -float(asem_tau), float(asem_tau))
                        if asem_variant in ("energy_normalized_boost", "clipped_energy_normalized_boost"):
                            base_energy = residual.abs().mean(dim=(1, 2, 3), keepdim=True)
                            mod_energy = residual_mod.abs().mean(dim=(1, 2, 3), keepdim=True)
                            residual_mod = residual_mod * (base_energy / torch.clamp(mod_energy, min=1e-8))
                    out_y = host_image + residual_mod
                encoded_image = self.bitencoder(out_y, message)          
                if return_fragile_carrier:
                    return {
                        "fragile_carrier": fragile_carrier,
                        "carrier_for_bitencoder": out_y,
