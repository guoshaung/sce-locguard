...
class Model_VSN(BaseModel):
    ...
    def feed_data(self, data):
        ...
        for key in ("GT_MASK", "gt_mask", "mask", "MASK"):
            if key in data and data[key] is not None:
                self.gt_mask = data[key].to(self.device).float()
                break
        for key in ("SEMANTIC_MAP", "semantic_map"):
            if key in data and data[key] is not None:
                self.semantic_map = data[key].to(self.device).float()
                break
        # self.mes = data['MES']

    def _train_flag(self, name, default=False):
        value = self.train_opt.get(name, default)
        return bool(value) if value is not None else bool(default)

    def _train_float(self, name, default=0.0):
        value = self.train_opt.get(name, default)
        return float(value) if value is not None else float(default)

    def _apply_freeze_options(self):
        if not self.is_train:
            return
        freeze_bitencoder = self._train_flag("freeze_bitencoder")
        freeze_bitdecoder = self._train_flag("freeze_bitdecoder")
        freeze_copyright_branch = self._train_flag("freeze_copyright_branch")
        if freeze_copyright_branch:
            freeze_bitencoder = True
            freeze_bitdecoder = True
        module = self.netG.module if isinstance(self.netG, (nn.DataParallel, DistributedDataParallel)) else self.netG
        if freeze_bitencoder and hasattr(module, "bitencoder"):
            for param in module.bitencoder.parameters():
                param.requires_grad = False
        if freeze_bitdecoder and hasattr(module, "bitdecoder"):
            for param in module.bitdecoder.parameters():
                param.requires_grad = False

    def _regularization_map(self, mask, residual):
        if mask is None:
            return None
        if mask.dim() == 5:
            mask = mask[:, 0]
        ...
        if mask.shape[-2:] != residual.shape[-2:]:
            mask = torch.nn.functional.interpolate(mask, size=residual.shape[-2:], mode="nearest")
        if mask.max() > 1:
            mask = mask / 255.0
        return torch.clamp(mask, 0.0, 1.0)

    def compute_carrier_regularization_losses(self, residual, gt_mask=None, semantic_map=None):
        zero = residual.sum() * 0.0
        abs_residual = torch.abs(residual)
        gt_mask = self._regularization_map(gt_mask, residual)
        semantic_map = self._regularization_map(semantic_map, residual)
        losses = {
            "carrier_sparsity_loss": abs_residual.mean(),
            "background_residual_suppression_loss": zero,
            "residual_floor_loss": zero,
            "semantic_guided_residual_loss": zero,
        }
        if gt_mask is not None:
            outside = 1.0 - gt_mask
            losses["background_residual_suppression_loss"] = (outside * abs_residual).mean()
            tau = self._train_float("residual_floor_tau", 0.0)
            small_residual_mask = (abs_residual < tau).to(abs_residual.dtype) if tau > 0.0 else torch.zeros_like(abs_residual)
            losses["residual_floor_loss"] = (outside * small_residual_mask * abs_residual).mean()
        if semantic_map is not None:
            losses["semantic_guided_residual_loss"] = ((1.0 - semantic_map) * abs_residual).mean()
        total = (
            self._train_float("lambda_sparse") * losses["carrier_sparsity_loss"]
            + self._train_float("lambda_bg") * losses["background_residual_suppression_loss"]
            + self._train_float("lambda_floor") * losses["residual_floor_loss"]
            + self._train_float("lambda_sem") * losses["semantic_guided_residual_loss"]
        )
        losses["carrier_regularization_total"] = total
        return losses
    ...
    def optimize_parameters(self, current_step):
        ...
        return_carrier = self._train_flag("return_fragile_carrier") or self._train_flag("enable_carrier_regularization")
        forw_result = self.netG(
            x=dwt(self.host.reshape(b, -1, h, w)),
            x_h=dwt(self.secret[:,0].reshape(b, -1, h, w)),
            message=message,
            return_fragile_carrier=return_carrier,
        )
        carrier_payload = None
        if isinstance(forw_result, dict):
            carrier_payload = forw_result
            self.output = forw_result["fragile_carrier"]
            container = forw_result["encoded_image"]
        else:
            self.output, container = forw_result
        ...
        carrier_reg_losses = None
        if self._train_flag("enable_carrier_regularization") and carrier_payload is not None:
            carrier_reg_losses = self.compute_carrier_regularization_losses(
        ...
        if self.mode == "image":
            ...
            if carrier_reg_losses is not None:
                loss = loss + carrier_reg_losses["carrier_regularization_total"]
            ...
            if carrier_reg_losses is not None:
                self.log_dict['l_carrier_sparse'] = carrier_reg_losses["carrier_sparsity_loss"].item()
                self.log_dict['l_carrier_bg'] = carrier_reg_losses["background_residual_suppression_loss"].item()
                self.log_dict['l_carrier_floor'] = carrier_reg_losses["residual_floor_loss"].item()
                self.log_dict['l_carrier_sem'] = carrier_reg_losses["semantic_guided_residual_loss"].item()
                self.log_dict['l_carrier_total'] = carrier_reg_losses["carrier_regularization_total"].item()
    ...
    def image_hiding(
        ...
        asem_alpha=0.0,
        asem_mode="none",
        asem_variant="naive_boost",
        asem_tau=0.0,
        asem_smooth_sigma=0.0,
        return_fragile_carrier=False,
    ):
        self.netG.eval()
        with torch.no_grad():
            b, t, c, h, w = self.real_H.shape
            ...
            forw_result = self.netG(
                ...
                message=message,
                asem_map=asem_map,
                asem_alpha=asem_alpha,
                asem_mode=asem_mode,
                asem_variant=asem_variant,
                asem_tau=asem_tau,
                asem_smooth_sigma=asem_smooth_sigma,
                return_fragile_carrier=return_fragile_carrier,
            )
            carrier_payload = None
            if isinstance(forw_result, dict):
                carrier_payload = forw_result
            ...
            if return_fragile_carrier:
                return lr_img, carrier_payload
            return lr_img
